...
import { Menu, Bell, Search, User, Moon, Sun, ArrowUpRight, ArrowDownLeft, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ManualEntryPage = () => {
  ...
  const handleVehicleEntry = async (license_plate, vehicle_type) => {
    try {
      ...
      console.log("=", entryLicensePlate, entryVehicleType);
  ...
  const handleVehicleExit = async (license_plate) => {
    try {
        ...
        <header className={`${darkMode ? 'bg-gray-800' : 'bg-white'}  shadow-md relative`}>
          <div className="flex items-center justify-between p-4">
            <div className="flex items-center">
              ...
              <h1 className={`text-xl font-semibold ${darkMode ? 'text-white' : 'text-gray-800'} ml-2`}>Manual Vehicle Entry</h1>
            </div>
